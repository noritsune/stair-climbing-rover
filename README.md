...
<img width="192" height="342" alt="moving" src="https://github.com/user-attachments/assets/3655d808-4ffa-4c59-87b9-29f3721179a7" />
...
> GPIO15 / GPIO5 はブートストラップピンだが、いずれも内部プルアップで起動時 HIGH が既定であり、
> PWM 出力は `setup()` の `ledcAttach` 後に始まるため通常は問題ない。書き込みが不安定な場合は
> `RoverConfig.h` の `EYE_SERVO_PIN[]` で別ピンに変更すること。
...
| `Options` | 全ステアと視線を 0° にリセンター |
| 十字キー 上下左右 | 目玉サーボの上下 / 左右（斜め入力で両軸同時。離すとセンターへ自動復帰）|
| `R2`（アナログ）| まぶた開閉。離す = 全開 / 全押し = 全閉（押し込み量に比例）|
| `L1` + 十字 / `△✕○□` | 配線確認モード（スティック中立時に各輪を個別に全力回転）|
...
| `EYE_TRIM_DEG[]` | 目玉サーボ（左右/上下/まぶた）の機械的ゼロ点オフセット（度）|
| `EYE_SERVO_REVERSED[]` | 目玉サーボのホーンが逆向きの軸を反転 |
| `EYE_MIN_OUTPUT_DEG[]` / `EYE_MAX_OUTPUT_DEG[]` | 目玉サーボ各軸（左右/上下/まぶた）の可動範囲。センター非対称でよい（既定 左右 ±30° / 上下 −45°〜+20°）|
| `EYELID_OPEN_DEG` / `EYELID_CLOSED_DEG` | まぶた全開／全閉の角度（既定 −45° / +30°）。`EYE_*_OUTPUT_DEG[EYE_LID]` から参照される |
| `EYE_RATE_DEG_PER_SEC` | 十字キー押下中の視線の移動速度 |
| `EYE_RETURN_RATE_DEG_PER_SEC` | 十字キーを離した後、センターへ自動復帰する速度 |
...
目玉サーボはギアなし直結のため `servo = outputDeg + 90`（センター 90°）。GPIO は `EYE_SERVO_PIN[]` = 左右 13 / 上下 15 / まぶた 5。

十字キーの斜め入力（右上など）は左右・上下の両軸を同時に動かす。DS4 の十字キーはハットスイッチで、斜めのときは `PS4.Up()` / `PS4.Right()` が両方 false になり `PS4.UpRight()` が true になるため、`.ino` 側で成分に展開している。
